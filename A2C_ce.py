...
if __name__ == "__main__":
    ...
    for run in range(NUM_RUNS):
        scores, eval_scores, eval_steps = run_reinforce_with_Net(seed=run, entropy=True, clip=True)
    ...
    df_eval.to_csv('./results/reinforce_a2c_clip_e_score.csv', index=False)
    ...
    os.makedirs('./results', exist_ok=True)
    df.to_csv('./results/reinforce_a2c_clip_e_results.csv', index=False)
