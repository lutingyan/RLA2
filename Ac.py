...
def AC():
    ...
    for episode in range(max_episodes):
        ...
        steps_per_episode.append(episode_steps)  # 记录当前episode的步数
        ...
        if episode % 20 == 0:
            avg_score = np.mean(scores[-20:]) if episode >= 20 else np.mean(scores)
            print(f"Episode {episode}, Steps: {episode_steps}, Score: {scores[-1]}, Avg: {avg_score:.1f}")

    return scores, steps_per_episode


if __name__ == "__main__":
    scores, steps_per_episode = AC()
    cumulative_steps = np.cumsum(steps_per_episode)  # 计算累计步数

    plt.figure(figsize=(10, 5))
    plt.plot(cumulative_steps, scores, label='Episode Score')  # 使用累计步数作为横轴
    ...
    plt.xlabel('Environment Steps')  # 横轴标签改为Environment Steps
    plt.ylabel('Score')
    plt.legend()
