...
def run_reinforce(fixed_lr, seed=0):
...
if __name__ == "__main__":
    ...
    for lr in lrs:
        ...
        for run in range(NUM_RUNS):
            scores, steps = run_reinforce(fixed_lr=lr, seed=run)
    ...
    for lr in lrs:
        df = pd.DataFrame({
            ...
            'cum_steps': results[lr]['cum_steps']
        })
        dfs.append(df)

    final_df = pd.concat(dfs)
    os.makedirs('results', exist_ok=True)
    csv_path = 'results/reinforce_lr_results.csv'
    final_df.to_csv(csv_path, index=False)

    print(f"\nResults saved to {csv_path}")
    print("\nSummary:")
    print(final_df.groupby('lr')['avg_reward'].agg(['mean', 'max']))