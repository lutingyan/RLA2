...
def run_reinforce():
    ...
    optimizer = optim.Adam(policy.parameters(), lr=lr)
    scores = []
    steps_per_episode = []  # 记录每个 episode 的步数
    ...
    for episode in range(max_episodes):
        ...
        scores.append(sum(episode_data['rewards']))
        steps_per_episode.append(steps)  # 记录当前 episode 的步数
        ...
        if episode % 50 == 0:
            avg_score = np.mean(scores[-50:]) if episode >= 50 else np.mean(scores)
            print(f'Episode {episode}, Steps: {steps}, Score: {scores[-1]}, Avg: {avg_score:.1f}')
...
if __name__ == "__main__":
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(cumulative_steps, scores, label='Episode Score')  # 横轴改为累计步数
    ...
    plt.xlabel('Environment Steps')  # 横轴标签改为 Environment Steps
    plt.ylabel('Score')
    plt.legend()
    plt.title('REINFORCE on CartPole-v1 (Step-based)')
    plt.grid(True)
    plt.show()
